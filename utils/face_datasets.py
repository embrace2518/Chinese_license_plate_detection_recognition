...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):  # LoadImagesAndLabels 类是用来读取和预处理图像及其对应的标签。
        # augment = augment：是否使用数据增强
        # hyp = hyp：数据增强的超参数
        # rect = rect：是否使用矩形训练
        # cache_images = cache：是否缓存图像
        # single_cls = opt.single_cls：是否启用单类别模式
        # image_weights = image_weights：是否根据类别权重进行采样
    ...
    # 如果是分布式训练 (rank != -1)，则使用 DistributedSampler 进行数据分配，否则不使用采样器。
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache')):
        ...
        # tqdm() 是一个进度条工具，可以在循环时显示进度，特别适用于数据加载、模型训练等任务。
        # pbar 是 tqdm 进度条对象，它封装了 zip(self.img_files, self.label_files)，每次迭代，它都会返回一对 (图像文件, 标签文件)。
