...
class small_basic_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.block = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(ch_out // 4, ch_out, kernel_size=1),  # 确保输出通道与定义一致
        )
...
class LPRNet(nn.Module):
    def __init__(self, lpr_max_len, num_classes, dropout_rate, export=False):
        ...
        self.backbone = nn.Sequential(
            ...
            small_basic_block(ch_in=64, ch_out=128),  # 4 → 输出128通道
            nn.BatchNorm2d(num_features=128),
            nn.ReLU(),  # 6
            nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(2, 1, 2)),
            small_basic_block(ch_in=128, ch_out=256),  # 8 → 修正输入为前层输出128通道
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(),  # 10
            small_basic_block(ch_in=256, ch_out=256),  # 11 → 保持256通道
            nn.BatchNorm2d(num_features=256),  # 12
            nn.ReLU(),
            nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(4, 1, 2)),  # 14
            nn.Dropout(dropout_rate),
            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(1, 4), stride=1),  # 16 → 输入修正为256
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(),  # 18
            nn.Dropout(dropout_rate),
    ...
    def forward(self, x):
        ...
        for i, f in enumerate(keep_features):
            for i, f in enumerate(keep_features):
                # 修改池化方式为自适应平均池化，统一特征图尺寸
                if i in [0, 1]:
                    f = nn.AdaptiveAvgPool2d((1, 1))(f)  # 原为AvgPool2d(kernel_size=5, stride=5)
                elif i == 2:
                    f = nn.AdaptiveAvgPool2d((1, 1))(f)  # 原为AvgPool2d(kernel_size=(4, 10), stride=(4, 2))
            f_pow = torch.pow(f, 2)
