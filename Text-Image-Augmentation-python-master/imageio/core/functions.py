...
Functions for reading:
  ...
  * :func:`.mimread` - read a series of rec_images from the specified uri
...
Functions for saving:
  ...
  * :func:`.mimwrite` - write a series of rec_images to the specified uri
...
All read-functions return rec_images as numpy arrays, and have a ``meta``
...
* ``get_reader("<video0>")`` to grab rec_images from a (web) camera.
...
def get_reader(uri, format=None, mode="?", **kwargs):
    ...
    mode : {'i', 'I', 'v', 'V', '?'}
        ...
        "i" for an image, "I" for multiple rec_images, "v" for a volume,
...
def mimread(uri, format=None, memtest=MEMTEST_DEFAULT_MIM, **kwargs):
    ...
    Reads multiple rec_images from the specified file. Returns a list of
    numpy arrays, each with a dict of meta data at its 'meta' attribute.

    Parameters
    ----------
    uri : {str, pathlib.Path, bytes, file}
        The resource to load the rec_images from, e.g. a filename,pathlib.Path,
        http address or file object, see the docs for more info.
    format : str
        The format to use to read the file. By default imageio selects
        the appropriate for you based on the filename and its contents.
    memtest : {bool, int, float, str}
        ...
        list of rec_images consumes greater than the amount of memory specified.
...
def mimwrite(uri, ims, format=None, **kwargs):
    ...
    Write multiple rec_images to the specified file.

    Parameters
    ----------
    uri : {str, pathlib.Path, file}
        The resource to write the rec_images to, e.g. a filename, pathlib.Path
        or file object, see the docs for more info.
    ...
    with writer:
        ...
        # Iterate over rec_images (ims may be a generator)
        for im in ims:
    ...
    if not written:
        raise RuntimeError("Zero rec_images were written.")
