...
# Example rec_images that will be auto-downloaded
EXAMPLE_IMAGES = {
    ...
    def _parse_uri(self, uri):
        ...
        if isinstance(uri, str):
            ...
            if uri.startswith("imageio:"):
                if is_write_request:
                    raise RuntimeError("Cannot write to the standard rec_images.")
                ...
                self._filename = get_remote_file("rec_images/" + fn, auto=True)
                if zip_part:
        ...
        # Deprecated way to load standard rec_images, give a sensible error message
        if is_read_request and self._uri_type in [URI_FILENAME, URI_ZIPPED]:
            ...
            if (not os.path.exists(fn)) and (fn in EXAMPLE_IMAGES):
                raise IOError(
                    ...
                    "the standard rec_images, but from imageio 2.1, "
                    "standard rec_images have to be specified using "
                    '"imageio:%s".' % (fn, fn)
                )
