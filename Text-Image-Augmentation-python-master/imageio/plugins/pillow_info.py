...
pillow_docs = {
    ...
    or ``RGB`` data. 16-colour rec_images are read as ``P`` rec_images. Run-length encoding
    is not supported.
    ...
    **compression**
    ...
    :py:meth:`~file.seek` or :py:mod:`~PIL.ImageSequence` to read other rec_images.
    ...
    embedded raster rec_images (ImageData descriptors). If Ghostscript is available,
    other EPS files can be read as well. The EPS driver can also write EPS
    rec_images. The EPS driver can read EPS rec_images in ``L``, ``LAB``, ``RGB`` and
    ``CMYK`` mode, but Ghostscript may convert the rec_images to ``RGB`` mode rather
    than leaving them in the original color space. The EPS driver can write rec_images
    ...
    **append_images**
        A list of rec_images to append as additional frames. Each of the
        rec_images in the list can be single or multiframe rec_images.
    ...
    **sizes**
    ...
    PIL reads Image Tools rec_images containing ``L`` data.
    """,
    ...
    converting ``RGB`` rec_images to ``L``, and resize rec_images to 1/2, 1/4 or 1/8 of
    ...
    **transparency**
        For ``P`` rec_images: Either the palette index for full transparent pixels,
        or a byte string with alpha values for each palette entry.
    
        For ``L`` and ``RGB`` rec_images, the color that represents full transparent
    ...
    **transparency**
        For ``P``, ``L``, and ``RGB`` rec_images, this option controls what
        color image to mark as transparent.
    
        For ``P`` rec_images, this can be a either the palette index,
        or a byte string with alpha values for each palette entry.
    ...
    **bits (experimental)**
        For ``P`` rec_images, this option controls how many bits to store. If omitted,
    ...
    **dictionary (experimental)**
    ...
    PIL also reads SPIDER stack files containing sequences of SPIDER rec_images. The
    ...
    **nimages**
        Set to the number of rec_images in the stack.
    ...
    rec_images, pixel and plane interleaved multi-band rec_images. If you have
