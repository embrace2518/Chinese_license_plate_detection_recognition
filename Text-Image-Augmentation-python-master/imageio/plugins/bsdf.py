...
class Image:
    ...
    we can make BSDF trigger on these classes and thus encode the rec_images.
    as actual rec_images.
    """
...
class BsdfFormat(Format):
    ...
    BSDF serialization format. This format allows storage of rec_images, volumes,
    ...
    not read until it is requested. This allows storing multiple rec_images
    in a single file and still have fast access to individual rec_images.
    Alternatively, a series of rec_images can be read in streaming mode, reading
    rec_images as they are read (e.g. from http).
    ...
    ----------------------
    random_access : bool
        Whether individual rec_images in the file can be read in random order.
    ...
    class Reader(Format.Reader):
        def _open(self, random_access=None):
            # Validate - we need a BSDF file consisting of a list of rec_images
            ...
            # random access to the rec_images.
