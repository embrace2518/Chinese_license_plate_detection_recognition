...
In desperation of wanting to share animated rec_images, and then lacking a good
...
of rec_images. The rec_images are stored using the DEFLATE algorithm (same as
...
GIF rec_images, we obtain better quality (24 bit colors + alpha channel)
...
# todo: use Pillow to support reading JPEG rec_images from SWF?
...
def read_pixels(bb, i, tagType, L1):
    ...
    if format != 5:
        logger.warning("Can only read 24bit or 32bit RGB(A) lossless rec_images.")
    else:
        # Read byte data
...
def checkImages(images):  # pragma: no cover
    """ checkImages(rec_images)
    Check numpy rec_images and correct intensity range etc.
...
def write_swf(filename, images, duration=0.1, repeat=True):  # pragma: no cover
    """Write an swf-file from the specified rec_images. If repeat is False,
    ...
    if hasattr(duration, "__len__"):
        if len(duration) == len(images2):
            ...
            raise ValueError("len(duration) doesn't match amount of rec_images.")
    else:
...
def read_swf(filename):  # pragma: no cover
    """Read all rec_images from an SWF (shockwave flash) file. Returns a list
    of numpy arrays.

    Limitation: only read the PNG encoded rec_images (not the JPG encoded ones).
    """
