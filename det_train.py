...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # save_dir: YOLOv5 默认的训练结果保存目录是 runs/train/
    # rank: 全局排名，常用于分布式训练中标识不同的节点。
    ...
    with torch_distributed_zero_first(rank):  # 确保只有排名为0的进程优先访问资源（例如检查数据集），以避免多进程同时访问导致冲突。
...
# ... existing code ...
    ...
    imgsz, imgsz_test = [check_img_size(x, grid_size) for x in opt.img_size]  # verify imgsz are gs-multiples
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, grid_size, opt,
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, grid_size, opt,  # testloader
